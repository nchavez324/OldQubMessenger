...
+ (UIImage *)imageWithBorderFromImage:(UIImage *)src withColor:(UIColor *)color lineWidth:(CGFloat)width{
...
+ (UIImage *)imageWithGlow:(UIImage *)src{
	CGFloat off = 6.0;
	CGSize newSize = CGSizeMake(src.size.width + off, src.size.height + off);
	CGRect newSizeRect = CGRectMake(0, 0, src.size.width + off, src.size.height + off);
	CGImageRef img = src.CGImage;
	
	UIGraphicsBeginImageContext(newSize);
	CGContextRef ctx = UIGraphicsGetCurrentContext();
	
	CGContextBeginTransparencyLayerWithRect(ctx, newSizeRect, NULL);
	
	//repeat?
	CGContextDrawImage(ctx, newSizeRect, img);
	CGContextSetAlpha(ctx, 0.2);
	
	CGContextEndTransparencyLayer(ctx);
	
	CGRect centerRect = CGRectMake(off/2.0, off/2.0, src.size.width, src.size.height);
	CGContextDrawImage(ctx, centerRect, img);
